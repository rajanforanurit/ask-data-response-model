...
const SEARCH_API_VERSION    = '2026-13-05'
...
// ══════════════════════════════════════════════════════════════════════════════
// ✅ NEW — Azure AI Search replaces loadChunksForClient + retrieveChunks
// ══════════════════════════════════════════════════════════════════════════════
async function searchChunks(clientId, query, topK = 6) {
  ...
  const searchBody = {
    search:  query,                                    // BM25 keyword part
    filter:  `client_id eq '${clientId}'`,             // tenant isolation
  ...
  if (queryVec) {
    searchBody.vectorQueries = [{
      ...
      vector: queryVec,
      fields: 'embedding',
      k:      topK * 3,    // oversample then semantic reranker trims to topK
    }]
  }
...
function buildContext(hits) {
  ...
  const deduped = []
  for (const h of hits) {
    // ✅ Deduplicate by chunk_index+source (not text prefix which broke on CHUNK_OVERLAP)
...
function buildSystemPrompt() {
  return `You are a helpful data assistant for a real estate analytics platform.
...
TABULAR DATA RULES:
11. When context contains rows like "City: X, Country: Y, Region: Z" treat each as structured data.
12. For lookup questions ("what country is Karachi in"), find the matching row and state the value directly. Example: "Karachi is located in Pakistan."
13. For count questions ("how many cities"), count distinct rows and state the number.
14. For ranking questions ("which city has highest visitors"), compare the values across rows and name the winner.
15. For aggregation ("total visitors in India"), sum values from all matching rows.
...
// ── Fallback answer (when Phi-4 is blank/unavailable) ─────────────────────────
function buildFallbackAnswer(query, hits) {
  ...
  // Tabular row lookup — scan all chunks for matching rows
  ...
  if (allRows.length > 0) {
    // Find rows that match any word from the query
    ...
    // Generic: return the best matching row as a readable sentence
...
app.get('/health', (req, res) => res.json({
  ...
  embeddings:        AZURE_EMBED_ENDPOINT ? 'azure' : 'none',
...
app.post('/client/login', async (req, res) => {
  try {
    ...
    // No warmup needed — Azure AI Search is always ready
...
// ══════════════════════════════════════════════════════════════════════════════
// ✅ MAIN CHAT ENDPOINT — now uses Azure AI Search
// ══════════════════════════════════════════════════════════════════════════════
app.post('/chat/message', requireClientKey, withRequestTimeout(async (req, res) => {
  try {
    ...
    const requestPromise = (async () => {
      ...
      // ✅ Azure AI Search — replaces loadChunksForClient + retrieveChunks
      let hits = []
      try {
...
app.listen(PORT, () => {
  ...
  console.log(`Embeddings: ${AZURE_EMBED_ENDPOINT ? 'Azure' : 'DISABLED — keyword+BM25 only'}`)
