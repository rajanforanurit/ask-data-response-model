...
for (const [regex, score] of patterns) { if (regex.test(h)) return score }
return 0
}
function detectColumns(headers) {
const NAME_PATTERNS = [[/\b(measure|attribute|field|metric|kpi)\s*name\b/, 100], [/^name$/, 90], [/report.?name/i, 85], [/\bname\b/, 70], [/\btitle\b/, 50]]
const TABLE_PATTERNS = [[/\b(table|module|category|group|domain|section|workspace)\b/, 100], [/^table$/, 90]]
const DESC_PATTERNS = [[/\b(description|desc|definition|about|summary|detail)\b/, 100], [/^desc$/, 90]]
const FORMULA_PATTERNS = [[/\b(formula|calculation|calc|how\s+calculated|computed\s+as)\b/, 100], [/^formula$/, 90]]
...
const pairLine = cells.map((val, j) => { if (!val) return ''; const key = headers[j] || `Field${j + 1}`; return `${key}: ${val}` }).filter(Boolean).join(' | ')
if (pairLine) parts.push(pairLine)
}
parts.push('')
rowsEmitted++
}
...
return `[Excerpt ${i + 1} | type: ${classifyExtension(h.source_file || '')} | score: ${typeof h._score === 'number' ? h._score.toFixed(3) : 'n/a'}]\n${(h.text || '').trim().slice(0, limit)}`
}).join('\n\n---\n\n')
}
...
const parts = []
for await (const chunk of download.readableStreamBody) parts.push(Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk))
...
if (!apiKey) { apiKey = generateApiKey() } else if (!apiKey.startsWith('rak_')) { return res.status(400).json({ error: 'apiKey must start with "rak_"' }) }
const database = await getDb()
...
if (intent === 'greeting') {
return res.json({ answer: "Hello! I'm your document assistant. Ask me anything about your documents, measures, attributes, or reports.", sources: [], conversationId: conversationId || null, client: { clientId, name } })
}
...
const updated = await col.findOneAndUpdate({ _id: new ObjectId(activeConversationId), clientId }, { $push: { messages: { $each: [userMsg, assistantMsg] } }, $set: { updatedAt: now } }, { returnDocument: 'after', projection: { _id: 1 } })
...
let hits = await retrieveChunks(query.trim(), chunks, Math.min(topK, 10), invertedIndex)
if (hits.length === 0) hits = relaxedKeywordSearch(query.trim(), chunks, 16, invertedIndex)
