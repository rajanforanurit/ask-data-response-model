...
// ─── REPLACEMENT for buildContextUD ─────────────────────────────────
function buildContextUD(hits) {
  ...
  return deduped.map((h, i) => {
    ...
    return cleanChunkText((h.text || '').trim()).slice(0, limit)
  }).join('\n\n---\n\n')
}
...
// ─── REPLACEMENT for buildFallbackAnswerUD ──────────────────────────
function buildFallbackAnswerUD(query, hits) {
  const { ensureSinglePeriod, trimToCompleteSentence, capFirst, escapeRegex, extractSubject } = require('./config')
  ...
  for (const h of hits.slice(0, 6)) {
    const text = cleanChunkText((h.text || '').trim())
...
module.exports = {
  slidingWindowChunk,
  buildInvertedIndexUD,
  retrieveChunksUD,
  preprocessQueryUD,
  buildSystemPromptUD,
  buildUserMessageUD,
  buildFallbackAnswerUD,
  cleanOcrNoise,
}
