...
{id:'capabilities_general',patterns:[/what\s+can\s+you\s+do/i,/what\s+are\s+your\s+(capabilities|features|functions|abilities)/i,/what\s+do\s+you\s+(support|offer|provide|handle)/i,/how\s+can\s+you\s+help(\s+me)?/i,/what\s+kind\s+of\s+questions\s+can\s+i\s+(ask|use)/i,/what\s+questions\s+can\s+i\s+ask/i,/what\s+type\s+of\s+questions/i,/what\s+topics\s+do\s+you\s+(cover|handle|support)/i,/what\s+can\s+i\s+(ask|use\s+you\s+for)/i,/what\s+(features|things)\s+do\s+you\s+(have|support|offer)/i,/how\s+do\s+you\s+work/i,/what\s+is\s+your\s+(purpose|goal|function|role)/i],responses:['Here\'s what I can help you with:\n\n- Data Analysis — Ask questions about your business metrics, KPIs, and trends.\n- Report Summarization — Get concise summaries of complex Power BI reports.\n- Context-Aware Querying — Retrieve specific data points from enterprise datasets.\n- Enterprise Insights — Uncover patterns, outliers, and actionable intelligence.\n- Power BI Integration — Natively embedded in your Power BI dashboards.\n\nJust type a business question and I\'ll find the answer from your data!']},
{id:'capabilities_powerbi',patterns:[/do\s+you\s+support\s+power\s*bi/i,/are\s+you\s+(a\s+)?power\s*bi/i,/power\s*bi\s+(support|integration|compatible|plugin|visual|tool|add-?in)/i,/is\s+this\s+(a\s+)?power\s*bi\s+(visual|tool|plugin|add-?in|report|dashboard)/i,/works?\s+with\s+power\s*bi/i,/power\s*bi\s+custom\s+visual/i,/embedded\s+in\s+power\s*bi/i,/can\s+i\s+use\s+(you|this)\s+(in|with|inside)\s+power\s*bi/i],responses:['Yes! Ask Data is built as a native Power BI Custom Visual. It embeds directly into your Power BI dashboards and reports, giving your team the ability to query enterprise data using natural language — no SQL or technical knowledge required.\n\nPowered by Anurit Innovation: https://www.anuritinnovation.com/']},
{id:'capabilities_analysis',patterns:[/can\s+you\s+(analyze|analyse)\s+data/i,/can\s+you\s+do\s+(data\s+)?(analysis|analytics)/i,/do\s+you\s+(analyze|analyse)\s+data/i,/are\s+you\s+able\s+to\s+analyze/i,/can\s+you\s+(find|detect|identify)\s+(trends?|patterns?|outliers?|insights?)/i,/can\s+you\s+(compare|show\s+me)\s+(data|numbers|metrics|kpis?)/i,/can\s+you\s+give\s+me\s+(insights?|analysis|analytics)/i],responses:['Absolutely! I\'m designed to analyze enterprise data in real time. You can ask things like:\n\n- "What were the top 5 products by revenue last quarter?"\n- "Show me the trend in customer acquisition over 6 months"\n- "Which region had the highest churn rate?"\n\nI retrieve answers directly from your connected data sources — no manual filtering needed.']},
{id:'capabilities_summarize',patterns:[/can\s+you\s+(summarize|summarise)\s+(reports?|dashboards?|data)/i,/can\s+you\s+summarize\s+data/i,/do\s+you\s+summarize/i,/report\s+summarization/i,/data\s+summary/i,/can\s+you\s+give\s+me\s+a\s+summary/i,/can\s+you\s+(summarize|summarise|recap|overview)/i,/can\s+you\s+(condense|digest|distill)\s+(this|data|reports?)/i],responses:['Yes! I can summarize reports and data for you. Just ask something like:\n\n- "Summarize this month\'s sales report"\n- "Give me an overview of operational KPIs"\n- "What are the key takeaways from the Q3 dashboard?"\n\nI\'ll distill the most important insights from your data into a clear, concise response.']},
{id:'capabilities_database',patterns:[/are\s+you\s+connected\s+to\s+(a\s+)?(database|db|data\s*source|backend)/i,/do\s+you\s+(connect|link)\s+to\s+a\s+(database|db)/i,/is\s+this\s+connected\s+to/i,/where\s+does\s+(your|the)\s+data\s+come\s+from/i,/what\s+(data|database|source)\s+are\s+you\s+(using|connected\s+to|pulling\s+from)/i,/do\s+you\s+(use|access)\s+(real|live|actual)\s+data/i,/how\s+do\s+you\s+(get|fetch|retrieve|access)\s+(the\s+)?data/i,/is\s+(the\s+)?data\s+(live|real.?time|updated|current)/i],responses:['Yes! Ask Data connects to your enterprise data sources through a secure RAG (Retrieval-Augmented Generation) pipeline. Your data is indexed and made searchable via vector embeddings, queries retrieve only the most relevant context, and responses are grounded in your actual enterprise data — not generic AI guesses. Your data stays within your organization\'s security perimeter.']},
{id:'thanks',patterns:[/^(thank\s*you|thanks|thank\s+u|thx|ty|cheers|great|awesome|perfect|nice|cool|got\s+it|ok(ay)?|sounds\s+good)\s*[!.,]*$/i,/^(thanks\s+a\s+lot|thank\s+you\s+so\s+much|much\s+appreciated)\s*[!.,]*$/i,/^(brilliant|wonderful|excellent|superb|amazing|fantastic)\s*[!.,]*$/i,/^(sure|alright|noted|understood|makes\s+sense)\s*[!.,]*$/i],responses:['You\'re welcome! Feel free to ask me anything about your data.','Happy to help! Let me know if you have any other questions.','Anytime! Ask me anything about your enterprise data or reports.','Glad I could help! What else would you like to explore?']},
